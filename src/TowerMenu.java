    ...
    public static void display(Controller controller) {
        ...
        buyLightTower.setOnAction(event -> {
            if (controller.getPlayer().isFull()) {
                AlertBox.display("Not Enough Space!",
                        "Either clear your destroyed towers or upgrade them!");
            } else if (currentFunds < lightTower.getBuyCost()) {
                AlertBox.display("Not Enough Funds!",
                        "You need more money!");
            } else if (tPlots.size() == 36) {
                AlertBox.display("Too many light towers",
                        "You have reached capacity for light towers!");
            }  else {
        ...
        upgradeLightTower.setOnAction(event -> {
            ...
            if (controller.getPlayer().getLTowers().size() == 0) {
                AlertBox.display("No tower to upgrade!",
                        "You need to have a light tower to upgrade.");
            } else if (currentFunds < lightTower.getUpgradeCost()) {
                AlertBox.display("Not Enough Funds!",
                        "You need more money!");
            } else if (tPlots.size() == 36) {
                AlertBox.display("Too many medium towers",
        ...
        upgradeMediumTower.setOnAction(event -> {
            ...
            if (controller.getPlayer().getMTowers().size() == 0) {
                AlertBox.display("No tower to upgrade!",
                        "You need to have a medium tower to upgrade.");
            } else if (tPlots.size() == 36) {
                AlertBox.display("Too many towers",
                        "You have reached capacity for towers!");
        ...
        layout.getChildren().addAll(currentMoney, towerShop,
                buyLightTower, buyLightPrice, lightTowerLabel,
                upgradeLightTower, upgradeLightPrice,  buyMediumTower,
                buyMediumPrice, mediumTowerLabel, upgradeMediumTower,
                upgradeMediumPrice, buyHeavyTower, buyHeavyPrice,
                heavyTowerLabel, closeButton);
